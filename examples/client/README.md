...
2. Persists a 32-byte salt (`channel.salt`) so the next run reuses the same channel
...
| `CLIENT_CHANNEL_STORAGE_DIR` | Disk dir for channel files. Default `./client-channels` |
| `CHANNEL_SALT` | Optional `0x` + 64 hex chars. If omitted, a random salt is written to `$CLIENT_CHANNEL_STORAGE_DIR/channel.salt` and reused |
...
Second process, same `.env` and same `client-channels/` dir: reuses the channel.

Point `RESOURCE_SERVER_URL` at another Sepolia seller (for example a shop) to act as a second independent buyer on the same rail. The facilitator URL is the **seller’s** problem — the buyer only hits the paid route.
